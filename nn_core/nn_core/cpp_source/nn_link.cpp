...
NN_Link* NN_Link::get_node(NN_Link* current_node) {
	return this;
}

void NN_Link::link_prev_node() {
	for (NN_Link* p_prev : _parent->_prev) {
		for (NN_Link* p_prev_child : p_prev->_child) {
			if (p_prev_child->is_selected) {
				p_prev_child->_next.push_back(this);
				_prev.push_back(p_prev_child);
				_input.push_back(&p_prev_child->_output);
				p_prev_child->_d_output.push_back(&_d_input);
...
/*
void NN_Link::set_child_link(NN_Link* current_child) {
	for (NN_Link* p_prev : current_child->_parent->_prev) {
