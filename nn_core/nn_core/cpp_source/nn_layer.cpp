...
NN_SoftMax::NN_SoftMax(int axis, const char* name) :
	NN_Layer(name),
	_axis(axis)
{
}

void NN_SoftMax::calculate_output_size(std::vector<Dimension*>& input_shape, Dimension& out_shape) {
	out_shape.clear();

	for (int i = 0; i < input_shape[0]->get_ranks(); ++i) {
		if (i != _axis) out_shape.push_back((*input_shape[0])[i]);
	}
}

void NN_SoftMax::build(std::vector<Dimension*>& input_shape) {

}

void NN_SoftMax::set_io(Dimension& out_shape, std::vector<DeviceTensor<nn_type>*>& input, DeviceTensor<nn_type>& output) {
	output.set(out_shape);
}

void NN_SoftMax::run_forward(cudaStream_t* s, std::vector<DeviceTensor<nn_type>*>& input, DeviceTensor<nn_type>& output) {
