...
class Tensor {
	...
	static void copy_unsorted_data(const _T* src, _T* dst, size_t size, const Vect1D& src_indice);
	static void copy_sorted_data(const _T* src, _T* dst, size_t size, const Vect1D& dst_indice);
	static void copy_unsorted_both_data(const _T* src, _T* dst, size_t size, const Vect1D& src_indice, const Vect1D& dst_indice);
	...
	static Vect1D generate_indice(const Vect1D& prev_indice, const Vect1D& steps, const Vect2D& dims_indice);
...
template <typename _T>
void Tensor<_T>::copy_unsorted_data(const _T* src, _T* dst, size_t size, const Vect1D& src_indice) {
...
template <typename _T>
void Tensor<_T>::copy_sorted_data(const _T* src, _T* dst, size_t size, const Vect1D& dst_indice) {
	tbb::parallel_for(tbb::blocked_range<size_t>(0, size), [&](const tbb::blocked_range<size_t>& q) {
...
template <typename _T>
void Tensor<_T>::copy_unsorted_both_data(const _T* src, _T* dst, size_t size, const Vect1D& src_indice, const Vect1D& dst_indice) {
	tbb::parallel_for(tbb::blocked_range<size_t>(0, size), [&](const tbb::blocked_range<size_t>& q) {
...
template <typename _T>
Vect1D Tensor<_T>::generate_indice(const Vect1D& prev_indice, const Vect1D& steps, const Vect2D& dims_indice) {
	cuint ranks = (uint)dims_indice.size();

	if (ranks == 0) return Vect1D();
	...
	Vect1D p_indice(size);

	if (prev_indice.empty()) {
	...
	_cnt_rank(new int(0))
{
	try {
		if (p._data != NULL) {
			...
			if (!_indice.empty())
				copy_unsorted_data(p._data.get(), _data.get(), _shape.total_size(), p._indice);
			else
...
cv::Mat Tensor<_T>::get_mat() const {
	...
	*_cnt_rank = 0;
	...
	if (_indice.empty()) memcpy(mat.ptr(), _data.get(), sizeof(_T) * _shape.total_size());
	else {
		copy_unsorted_data(
			_data.get(),
			(_T*)mat.ptr(),
...
Tensor<_cT> Tensor<_T>::cast() const {
	...
	if (_indice.empty()) {
		...
		tbb::parallel_for(tbb::blocked_range<size_t>(0, _shape.total_size()), [&](const tbb::blocked_range<size_t>& q) {
			for (size_t i = q.begin(); i < q.end(); ++i) p_dst[i] = (_cT)(p_src[_indice[i]]);
		}, tbb::auto_partitioner());
	}
...
const Tensor<_T>& Tensor<_T>::operator=(const Tensor& p) {
	...
	if (_data == NULL) {
		if (p._data != NULL) {
			...
			if (p._indice.empty()) memcpy(_data.get(), p._data.get(), sizeof(_T) * size);
			else copy_unsorted_data(p._data.get(), _data.get(), size, p._indice);
		}
	}
	else {
		if (p._data != NULL) {
...
const Tensor<_T>& Tensor<_T>::operator=(const GpuTensor<_T>& p) {
	...
	if (_data == NULL) {
		...
		if (_indice.empty())
			...
			copy_sorted_data(p_src, _data.get(), src_size, _indice);
...
const Tensor<_T>& Tensor<_T>::operator=(const _T& scalar) {
	...
	if (_indice.empty()) {
		...
		tbb::parallel_for(tbb::blocked_range<size_t>(0, size), [&](const tbb::blocked_range<size_t>& q) {
			for (size_t i = q.begin(); i < q.end(); ++i) dst[_indice[i]] = scalar;
		}, tbb::auto_partitioner());
	}
...
const Tensor<_T>& Tensor<_T>::operator=(const cv::Mat& mat) {
	...
	if (_data == NULL) {
		...
		if (_indice.empty()) memcpy(_data.get(), mat.ptr(), sizeof(_T) * shape.total_size());
		else {
			copy_sorted_data(
				(_T*)mat.ptr(),
				_data.get(),
