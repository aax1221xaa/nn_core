...
public:
	...
	Model(NN_Manager& manager, Layer_t inputs, Layer_t outputs, const std::string& model_name = "");
	Model(const Model& p);
	Model(Model&& p);
	~Model();

	const Model& operator=(const Model& p);
	const Model& operator=(Model&& p);
	...
	NN_List<Tensor<nn_type>> fit(const SampleGenerator<_xT, _yT>& samples, int batch, int steps);
};

template <typename _T>
NN_List<Tensor<nn_type>> Model::predict(const std::vector<Tensor<_T>>& x, int batch_size, int steps) {
	_manager.set_resize_shapes();
	_manager.set_resize_outputs();
	...
	for (NN_List<Tensor<nn_type>>& m_output : outputs) m_output.resize(_output_nodes.size());
	...
	for (int i = 0; i < steps; ++i) {
		...
		for (NN_Link* p_out_link : _output_nodes) {
			...
			//outputs[i][n].val() = Tensor<nn_type>(out_tensor.get_shape());
			outputs[i][n++].append(Tensor<nn_type>(out_tensor));
		}
	}
#if _DEBUG
...
template <typename _xT, typename _yT>
NN_List<Tensor<nn_type>> Model::fit(const SampleGenerator<_xT, _yT>& samples, int batch, int steps) {
	/*
