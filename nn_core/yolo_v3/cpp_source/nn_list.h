...
public:
	...
	public:
		...
		Iterator(NN_List& list, bool is_begin);
	...
	public:
		...
		ConstIterator(const NN_List& list, bool is_begin);
...
template <class _T>
NN_List<_T>::Iterator::Iterator(NN_List& list, bool is_begin) :
	_ref(list)
{
	if (list._data) {
		if (is_begin) _sw = 1;
	...
	else if (list._p_list) {
		if (is_begin) _iter = list._p_list->begin();
		else _iter = list._p_list->end();
...
bool NN_List<_T>::Iterator::operator!=(const typename NN_List::Iterator& p) {
	const int sw = _sw + p._sw;

	if (sw < 0) return _iter != p._iter;
	else if (sw > 0) return true;
	else return false;
}

template <class _T>
NN_List<_T>& NN_List<_T>::Iterator::operator*() {
	if (_sw > 0) return _ref;
	else return *(*_iter);
}

template <class _T>
NN_List<_T>::ConstIterator::ConstIterator(const NN_List& list, bool is_begin) :
	_ref(list)
{
	if (list._data) {
		if (is_begin) _sw = 1;
		...
		if (is_begin) _iter = list._p_list->cbegin();
		else _iter = list._p_list->cend();
...
typename NN_List<_T>::Iterator NN_List<_T>::begin() {
	return NN_List<_T>::Iterator(*this, true);
}

template <class _T>
typename NN_List<_T>::Iterator NN_List<_T>::end() {
	return NN_List<_T>::Iterator(*this, false);
}

template <class _T>
typename NN_List<_T>::ConstIterator NN_List<_T>::begin() const {
	return NN_List<_T>::ConstIterator(*this, true);
}

template <class _T>
typename NN_List<_T>::ConstIterator NN_List<_T>::end() const {
	return NN_List<_T>::ConstIterator(*this, false);
}
