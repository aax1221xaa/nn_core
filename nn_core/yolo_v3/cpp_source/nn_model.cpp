...
void Model::find_path(Layer_t& inputs, Layer_t& outputs, std::vector<int>& find_mask) {
	...
	for (Layer_t& ptr : outputs) p_outputs.push_back(ptr[0].val()._node);
...
void Model::set_shapes() {
	...
	else if (!(_status & 0x02)) _manager.set_resize_shapes();
...
Model::Model(NN_Manager& manager, const std::string& model_name) :
	_status(0),
	NN_Layer(model_name, "Model"),
	_manager(manager)
{
}

Model::Model(NN_Manager& manager, Layer_t inputs, Layer_t outputs, const std::string& model_name) :
	_status(0),
	NN_Layer(model_name, "Model"),
	_manager(manager)
{
...
Model::Model(const Model& p) :
	...
	_status(p._status),
	_manager(p._manager),
	_input_nodes(p._input_nodes),
	_output_nodes(p._output_nodes),
	_layers(p._layers),
	_losses(p._losses),
	_optimizer(p._optimizer)
{
...
const Model& Model::operator=(const Model& p) {
...
const Model& Model::operator=(Model&& p) {
	_layer_name = p._layer_name;
	_uniq_name = p._uniq_name;
...
NN_Link* Model::create_child() {
	...
	Model* child_model = new Model(_manager, _uniq_name);
...
Layer_t Model::operator()(Layer_t prev_node) {
	...
	if (!(_status & 0x02)) _manager.set_resize_shapes();
...
void Model::load_weights(const std::string& path, bool skip_mismatch) {
	...
	for (const std::string& name : layer_names) {
		for (NN_Link* p_link : _layers) {
			if (name == p_link->get_layer()._uniq_name) {
...
void Model::summary() {
	...
	for (NN_Link* p_node : _layers) {
		std::cout << std::left << std::setw(3) << ++i << 
			" : layer_name = " << std::setw(25) << p_node->get_layer()._uniq_name << 
			", output shape = " << nodes_shape[p_node->get_index()];
	}
}
