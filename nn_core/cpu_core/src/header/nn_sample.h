...
struct DataSet {
	std::vector<NN_Tensor<_xT>> _x;
	std::vector<NN_Tensor<_yT>> _y;
};
...
void Sample<_xT, _yT>::set_batch_samples(const DataSet<_xT, _yT>& origin, DataSet<_xT, _yT>& buff, int index, int n_batch, bool shuffle) {
	...
	typename std::vector<NN_Tensor<_xT>>::iterator x_iter = buff._x.begin();
	typename std::vector<NN_Tensor<_yT>>::iterator y_iter = buff._y.begin();

	for (const NN_List<NN_Tensor<_xT>>& x : origin._x) {
	...
	for (const NN_List<NN_Tensor<_yT>>& y : origin._y) {
