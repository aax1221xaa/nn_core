...
private:
	...
	*/
	...
	static void parsing_weight(const H5::Group& group, NN_List<NN_Tensor<nn_type>>& g_tensor);
	static NN_List<NN_Tensor<nn_type>> zero_clone(const NN_List<NN_Tensor<nn_type>>& tensor);
...
public:
	...
	NN_List<NN_Tensor<nn_type>> predict(const std::vector<NN_Tensor<_T>>& x, int batch_size, int steps);

	void stand_by(const NN_Optimizer& optimizer, const std::initializer_list<NN_Loss>& loss);

	template <typename _xT, typename _yT>
	NN_List<NN_Tensor<nn_type>> fit(const DataSet<_xT, _yT>& samples, int batch, int steps);
};

template <typename _T>
NN_List<NN_Tensor<nn_type>> Model::predict(const std::vector<NN_Tensor<_T>>& x, int batch_size, int steps) {
	_manager.set_reserved_shapes();
	_manager.set_reserved_outputs();

	NN_List<NN_Shape>& nodes_shapes = _manager.get_node_shape();
	NN_List<NN_Tensor<nn_type>>& nodes_outputs = _manager.get_node_output();
	NN_List<NN_Tensor<nn_type>> outputs;
	std::vector<NN_Tensor<_T>> batch_x(x.size());

	outputs.reserve(steps);
	
	for (NN_List<NN_Tensor<nn_type>>& m_output : outputs) m_output.resize(x.size());
	...
	for (int i = 0; i < steps; ++i) {
		if (i == 0) {
			...
			for (NN_Link* node : _layers) {
				...
				NN_List<NN_Tensor<nn_type>>& m_output = nodes_outputs[node->get_index()];
...
template <typename _xT, typename _yT>
NN_List<NN_Tensor<nn_type>> Model::fit(const DataSet<_xT, _yT>& samples, int batch, int steps) {
	/*
	_manager.set_reserved_shapes();
	_manager.set_reserved_outputs();
