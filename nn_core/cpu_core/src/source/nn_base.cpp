...
void NN_Layer::build(const NN_List<NN_Shape>& input_shape, NN_List<NN_Tensor<nn_type>>& weights) {

}

void NN_Layer::run(const NN_List<NN_Tensor<nn_type>>& input, NN_List<NN_Tensor<nn_type>>& output) {
	ErrorExcept(
...
NN_List<NN_Tensor<nn_type>> NN_Layer::get_weight() {
	return NN_List<NN_Tensor<nn_type>>();
}

void NN_Layer::set_output(const NN_List<NN_Shape>& output_shape, NN_List<NN_Tensor<nn_type>>& input, NN_List<NN_Tensor<nn_type>>& output) {
	for (const NN_List<NN_Shape>& shape : output_shape) output.append(NN_Tensor<nn_type>(shape.val()));
}
...
void NN_Input::set_output(const NN_List<NN_Shape>& output_shape, NN_List<NN_Tensor<nn_type>>& input, NN_List<NN_Tensor<nn_type>>& output) {
	if (input.size()) output.append(input[0].val());
	else output.append(NN_Tensor<nn_type>(output_shape[0].val()));
}
...
NN_List<NN_Tensor<nn_type>>& NN_Manager::get_weights() {
	return _weights;
}
...
NN_List<NN_Tensor<nn_type>>& NN_Manager::get_node_output() {
	return _outputs;
}

NN_List<NN_Tensor<nn_type>>& NN_Manager::get_node_doutput() {
	return _doutputs;
}
...
void set_random_uniform(NN_Tensor<nn_type>& tensor, nn_type min, nn_type max) {
	...
	cv::RNG rng(rd());
	size_t size = sizeof(nn_type) * tensor.get_shape().total_size();
	cv::Mat tmp(tensor.get_shape().get_dims(), CV_32FC1);

	rng.fill(tmp, cv::RNG::UNIFORM, min, max, true);
	memcpy_s(tensor.get_shared_ptr().get(), size, tmp.ptr<nn_type>(0), size);
}