...
void Model::parsing_weight(const H5::Group& group, NN_List<NN_Tensor<nn_type>>& g_tensor) {
	...
	for (const std::string& name : weight_names) {
		...
		NN_Tensor<nn_type> tensor(dims, n_dims);
...
NN_List<NN_Tensor<nn_type>> Model::zero_clone(const NN_List<NN_Tensor<nn_type>>& tensor) {
	NN_List<NN_Tensor<nn_type>> tmp;

	if (tensor.is_scalar()) {
		const NN_Shape shape = tensor.val().get_shape();

		tmp.append(NN_Tensor<nn_type>::zeros(shape));
	}
	else {
		for (const NN_List<NN_Tensor<nn_type>>& m_tensor : tensor) {
...
void Model::set_weights() {
	...
	NN_List<NN_Tensor<nn_type>> dummy_tensor;
...
void Model::build(const NN_List<NN_Shape>& input_shape, NN_List<NN_Tensor<nn_type>>& weights) {
	if (!(_status & 0x01)) put_error(_status);
		...
		NN_List<NN_Shape>& nodes_shape = _manager.get_node_shape();
		NN_List<NN_Tensor<nn_type>>& weights = _manager.get_weights();
...
void Model::run(const NN_List<NN_Tensor<nn_type>>& input, NN_List<NN_Tensor<nn_type>>& output) {
	NN_List<NN_Tensor<nn_type>>& nodes_output = _manager.get_node_output();

	for (NN_Link* p_node : _layers) {
		const int n_node = p_node->get_index();
		NN_List<NN_Tensor<nn_type>>& m_output = nodes_output[n_node];
		NN_List<NN_Tensor<nn_type>> m_input;
		...
		p_node->get_layer().run(m_input, m_output);
	}
}
...
void Model::set_output(const NN_List<NN_Shape>& output_shape, NN_List<NN_Tensor<nn_type>>& input, NN_List<NN_Tensor<nn_type>>& output) {
	...
	for (const std::string& name : layer_names) {
		for (NN_Link* p_link : _layers) {
			if (name == p_link->get_layer()._layer_name) {
				...
				NN_List<NN_Tensor<nn_type>> weight = p_link->get_layer().get_weight();
